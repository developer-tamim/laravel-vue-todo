            ...
            <table class="w-full bg-white border-collapse rounded-md shadow-lg">
                ...
                <tbody>
                    <tr v-for="(task, index) in tasks" :key="task.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 border">
                            <span
                                :class="task.status === 'Completed' ? 'text-green-600 font-semibold' : 'text-yellow-600 font-semibold'">
                                {{ task.status }}
                            </span>
                        </td>
                        <td class="flex items-center justify-center px-4 py-3 space-x-3 border">
                            ...
                            <button @click="deleteTask(task.id)" class="text-red-500 hover:underline">
                                <svg xmlns="http://www.w3.org/2000/svg" width="2000" height="24" viewBox="0 0 24 24"
                                    fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"
                                    stroke-linejoin="round"
            ...
            <div class="p-6 bg-white rounded-lg shadow-lg w-96">
                ...
                <form @submit.prevent="saveTask">
                    ...
                    <div class="mb-4">
                        ...
                        <input v-model="taskForm.title" id="taskTitle" type="text"
                            class="w-full p-2 mt-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
                            required />
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <select v-model="taskForm.status" id="taskStatus"
                            class="w-full p-2 mt-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
                            required>
                    ...
                    <div class="mb-4">
                        ...
                        <textarea v-model="taskForm.description" id="taskDescription" rows="3"
                            class="w-full p-2 mt-1 border rounded focus:outline-none focus:ring-2 focus:ring-blue-400"
                            required></textarea>
                    </div>
                    ...
                    <div class="flex justify-end space-x-3">
                        <button @click="closeModal" type="button"
                            class="px-4 py-2 text-gray-700 bg-gray-300 rounded">Cancel</button>
                        <button type="submit"
                            class="px-4 py-2 text-white bg-blue-500 rounded hover:bg-blue-600">Save</button>
                    </div>
                </form>
            </div>
        </div>
...
export default {
    ...
    methods: {
        ...
        saveTask() {
            if (this.isEditMode) {
                this.updateTask(this.taskForm.id, this.taskForm);
            } else {
                this.createTask();
            }
            this.closeModal();
        ...
        createTask() {
            this.$inertia.post(route('todos.store'), this.taskForm);
        },
